...
function getSearchEnabled(settings: any): boolean {
  return Boolean(settings?.plugins?.search?.enabled);
}
...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json({
      enabled: getSearchEnabled(keyData.fandom_settings)
    });
  } catch (err: any) {
    console.error('[GET Search Plugin] Error:', err);
...
export async function PATCH(
  ...
  try {
    ...
    const nextSettings = withSearchEnabled(keyData.fandom_settings, enabled);
    ...
    if (updateError) {
      console.error('[PATCH Search Plugin] Supabase update error:', updateError);
      return NextResponse.json({ error: 'Failed to persist plugin settings' }, { status: 500 });
    }

    return NextResponse.json({ success: true, enabled });
  } catch (err: any) {
    console.error('[PATCH Search Plugin] Error:', err);
    return NextResponse.json({ error: 'Internal Server Error' }, { status: 500 });
  }
}
