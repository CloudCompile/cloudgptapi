  ...
  try {
    ...
    if (event.type === 'invoice.payment_failed') {
      ...
      if (session.subscription) {
        ...
        if (updateError) {
          console.error(`[${requestId}] Error updating subscription status for failed payment:`, updateError);
        }
      }
    }
    ...
    if (event.type === 'customer.subscription.updated') {
      ...
      if (subData) {
        ...
        if (profileError) {
          console.error(`[${requestId}] Error updating profile plan on subscription update:`, profileError);
        }
        ...
        if (subError) {
          console.error(`[${requestId}] Error updating subscription on update:`, subError);
        }
      }
    }
    ...
    if (event.type === 'customer.subscription.deleted') {
      ...
      if (subQueryError || !subData) {
        ...
        if (subUpdateError) {
          console.error(`[${requestId}] Error updating subscription status to canceled:`, subUpdateError);
        }
      }
    }
  } catch (error: any) {
